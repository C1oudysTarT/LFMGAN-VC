...
class Collater(object):
    ...
    def __call__(self, batch):
        ...
        x_input = spec_augmentation(mels)

        # print(mels.equal(x_input))

        # plt.figure()
        # librosa.display.specshow(mels[0, 0, :, :].numpy(), x_axis='time', y_axis='mel', sr=48000, fmax=12000, cmap='viridis')
        # plt.colorbar(format='%+2.0f dB')
        # plt.savefig('x_real.png', dpi=600, bbox_inches='tight')
        # plt.close()
        # plt.figure()
        # librosa.display.specshow(x_input[0, 0, :, :].numpy(), x_axis='time', y_axis='mel', sr=48000, fmax=12000, cmap='viridis')
        # plt.colorbar(format='%+2.0f dB')
        # plt.savefig('x_input.png', dpi=600, bbox_inches='tight')
        # plt.close()
        return mels, labels, ref_mels, ref2_mels, ref_labels, z_trg, z_trg2, x_input
